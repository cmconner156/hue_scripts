#!/bin/bash
#Testing _get_workflows function
...
Testing get_workflows function:
...
LOG.info("Documents: Document2: workflow2: %s" % Document.objects.get_docs(user, Document2, extra='workflow2').__dict__)
for d in Document.objects.get_docs(user, Document2, extra='workflow2'):
  LOG.info("document: %s" % d.__dict__)
  LOG.info("document.content_object: %s" % d.content_object.__dict__)
...
subworkflows_json = json.dumps(subworkflows_json, cls=JSONEncoderForHTML)
LOG.info("subworkflows_json: %s" % subworkflows_json)
