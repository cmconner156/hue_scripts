...
parse_arguments()
{
  ...
  GETOPT=`getopt -n $0 -o u:,w:,s:,p:,t:,e,v,h \
      -l user:,password:,server:,port:,text:,enablessl,verbose,help \
      -- "$@"`
  eval set -- "$GETOPT"
  while true;
  do
...
API2 query on text from Hue server:
...
OPTIONS
   ...
   -t|--text		   Text to search for in doc2.
...
main()
{
   ...
   HUE_HISTORY_TEST_URL="${HUE_HTTP}://${HUE_SERVER}:${HUE_PORT}/notebook/api/get_history?doc_type=hive&limit=50&doc_text=${TEXT}"
   
   hue_login
   echo "Doing history GET on TEXT: ${TEXT}:"
   do_curl \
        GET \
        "${HUE_HISTORY_TEST_URL}" \
