...
parse_arguments()
{
  ...
  GETOPT=`getopt -n $0 -o u:,w:,s:,p:,f:,e,v,h \
      -l user:,password:,server:,port:,enablessl,srcfile:,verbose,help \
      -- "$@"`
  eval set -- "$GETOPT"
  while true;
  do
...
OPTIONS
   ...
   -f|--srcfile		   Source file to upload to HDFS
   -e|--enablessl	   Enable SSL HTTPS
...
main()
{
   ...
   HUE_FILEBROWSER_URL="${HUE_HTTP}://${HUE_SERVER}:${HUE_PORT}/filebrowser/upload/file?dest=/tmp"

   hue_login
   echo "Testing file upload to Hue file: ${SRC_FILE}"
   echo "$HUE_FILEBROWSER_URL"
   do_curl \
        POST \
        "${HUE_FILEBROWSER_URL}" \
        -L -v -F dest="/tmp" -F filename="$(basename ${SRC_FILE})" -F hdfs_file=@${SRC_FILE}
